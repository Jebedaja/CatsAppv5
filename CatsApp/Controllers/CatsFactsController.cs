...
namespace CatsApp.Controllers
{
    ...
    public class CatFactsController : Controller
    {
        ...
        {
            var catFacts = await GetCatFactsFromApi();              // pobieranie faktow o kotach

            return View("/Views/CatsFacts/Index.cshtml", catFacts);
        }

        [HttpGet]
        public async Task<IActionResult> GetNextFact()             // pobieranie kolejnego faktu o kotach  
        {
        ...
        private async Task<List<CatFact>> GetCatFactsFromApi()     // pobieranie listy faktow
        {
            var catFact = await GetCatFactFromApi();
